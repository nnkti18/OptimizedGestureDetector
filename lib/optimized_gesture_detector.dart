...
class OptimizedGestureDetector extends StatelessWidget {
    ...
    return gd.CoreGestureDetector(
      ...
      onTapUp: (details) {
        ...
        if (_isSingleTap) {
          ...
          int consumeInterval = DateTime.now().millisecondsSinceEpoch - _tapDownTime;
          _singleTapTimer = Timer(Duration(milliseconds: DOUBLE_TAP_INTERVAL + SINGLE_TAP_TRIGGER_TIME_BUFFER - consumeInterval), () {
            _isSingleTap = false;
            _singleTap(details);
            _cancelAndResetSingleTapTimer();
          });
      ...
      onScaleUpdate: (details) {
        ...
        if (_isRealScale == null) {
          _isRealScale = !(details.verticalScale == 1.0 && details.horizontalScale == 1.0 && details.rotation == 0.0);
          if (_isRealScale != null && _isRealScale!) {
            _moveCancel();
          } else {
            _scaleCancel();
          }
  ...
  void _moveEnd(scale.OpsSEndDetails details) {
    ...
    if (_moveEndCallback != null) {
      _moveEndCallback!(DetailsUtils.toOpsMoveEndDetails(details, _lastMoveUpdateGolbalPos ?? ZERO, _lastMoveUpdateLocalPos ?? ZERO));
    }
    _resetLastMoveUpdatePos();
  }
    ...
    if (details.globalPointerLocations!.length >= 2) {
      ...
      if (_scaleTwoKeys.length >= 2) {
        if (_scaleMainDirection == null) {
          _scaleMainDirection = getCurrentDirection(details.globalPointerLocations![_scaleTwoKeys[0]]!, details.globalPointerLocations![_scaleTwoKeys[1]]!);
        }

        Util.L2("direction $_scaleMainDirection");
        if (_scaleUpdateCallback != null) {
          _scaleUpdateCallback!(DetailsUtils.toOpsScaleUpdateDetails(details, _scaleMainDirection));
        }
      }
    }
  ...
  void _scaleEnd(scale.OpsSEndDetails details) {
    ...
    if (_scaleEndCallback != null) {
      _scaleEndCallback!(DetailsUtils.toOpsScaleEndDetails(details, _lastScaleUpdateGolbalPos ?? ZERO, _lastScaleUpdateLocalPos ?? ZERO));
    }
    _resetLastScaleUpdatePos();
  }
